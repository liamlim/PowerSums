...
\begin{document}
	...
	\subsection{Minimal polynomial $P(t)$}
	\begin{definition}
		\[P(t) = t^3 - e_1 t^2 + e_2 t - e_3.\]
		Thus $x$, $y$, and $z$ are the roots of $P(t) = 0$.
	\end{definition}
	
	\begin{lemma}[Roots $x,y,z$]
		Each of $x$, $y$, $z$ is a root of $P$:
		\begin{align*}
			P(x) = 0, \\
			P(y) = 0, \\
			P(z) = 0.
	...
	\end{lemma}
	\begin{proof}
		This follows immediately from the factorisation
		\[P(t) = (t - x)(t - y)(t - z).\]
	\end{proof}
	...
	\begin{lemma}[Isolating single powers]
		\begin{align*}
			x^n = f_n(1, 0, 0),\\
			y^n = f_n(0, 1, 0),\\
			z^n = f_n(0, 0, 1).
	...
	\end{theorem}
	\begin{proof}
		By the minimal polynomial lemma, each of $x, y, z$ satisfies $t^3 = e_1 t^2 - e_2 t + e_3$. Multiplying both sides by $t^n$ and summing over $x, y, z$ with weights $k, l, m$ yields the result.
	\end{proof}
	...
	\begin{proof}
		...
		for $f_2$ follows by straightforward algebra.
	\end{proof}
	...
	\begin{corollary}[Closed form for $K(n)$]
		For all integers $n$,
		\[
		-(x-y)(y-z)(z-x)\cdot K(n) = x^n(y-z) + y^n(z-x) + z^n(x-y).
		\]
	\end{corollary}
	\begin{proof}
		Expanding the numerator gives $f_n(w_x, w_y, w_z)$, and the
		denominator is $f_2(w_x, w_y, w_z) = -(y-z)(z-x)(x-y)$.
	\end{proof}
	...
	% -----------------------------------------------------------------------
	\section{Generalised Basis Functions $K_n(a,b,c)$}
	
	\subsection{Definition}
	
	The function $K(n)$ depends implicitly on the triple $(x, y, z)$ through
	the elementary symmetric polynomials $e_1, e_2, e_3$.  It is useful to
	make this dependence explicit by defining a version of $K$ for an
	arbitrary triple of distinct non-zero values.
	
	\begin{definition}
		For any triple $(a, b, c)$ of distinct non-zero reals and any integer
		$n$, define
		\[
		K_n(a,b,c)
	...
	Thus $K_n(a,b,c)$ is the function $K$ for the system whose roots are
	$a$, $b$, $c$; in particular $K_n(x,y,z) = K(n)$.  The denominator
	equals $f_2(b-c,\, c-a,\, a-b)$ computed for the triple $(a,b,c)$, and
	is non-zero precisely because $a, b, c$ are distinct.
	
	\begin{lemma}[Initial values]
		For any admissible triple $(a,b,c)$,
		\[
		K_0(a,b,c) = 0, \qquad K_1(a,b,c) = 0, \qquad K_2(a,b,c) = 1.
		\]
	\end{lemma}
	\begin{proof}
		The cases $n = 0$ and $n = 1$ follow from the same calculations as
		Lemma~3.1 (the numerator vanishes).  For $n = 2$ the numerator equals
		the denominator, so the ratio is $1$.
	\end{proof}
	
	\begin{theorem}[Recurrence for $K_n(a,b,c)$]
		Let $\varepsilon_1 = a+b+c$, $\varepsilon_2 = ab+bc+ca$,
		$\varepsilon_3 = abc$.  Then
		\[
		K_{n+3}(a,b,c)
		= \varepsilon_1\, K_{n+2}(a,b,c)
		- \varepsilon_2\, K_{n+1}(a,b,c)
		+ \varepsilon_3\, K_n(a,b,c).
		\]
	\end{theorem}
	\begin{proof}
		The proof is identical to that of the recurrence for $K(n)$: each of
		$a$, $b$, $c$ satisfies its own minimal polynomial, and the claim
		follows by the same linearity argument.
	\end{proof}
	
	\subsection{The squaring substitution and a doubling identity}
	
	A natural instance of $K_n(a,b,c)$ arises from the substitution
	$(a,b,c) = (x^2, y^2, z^2)$.  The symmetric polynomials of the squared
	triple are
	\[
	\varepsilon_1 = e_1^2 - 2e_2, \qquad
	\varepsilon_2 = e_2^2 - 2e_1 e_3, \qquad
	\varepsilon_3 = e_3^2.
	\]
	
	\begin{theorem}[Doubling identity for $L$]
		For all integers $n$,
		\[
	...
	\end{theorem}
	\begin{proof}
		Note that $e_3 - e_1 e_2 = -(x+y)(x+z)(y+z)$.  Both sides vanish at
		$n = 0$ and $n = 1$.  At $n = 2$ the right-hand side is $L(4)$, and
		a direct expansion confirms $L(4) = -(x+y)(x+z)(y+z)$, which equals
		$(e_3 - e_1 e_2)\cdot K_2(x^2,y^2,z^2) = e_3 - e_1 e_2$.  Both sides
		satisfy recurrences (the left side that of the $(x^2,y^2,z^2)$
		system scaled by the constant $e_3 - e_1 e_2$; the right side the
		original recurrence applied at even indices), and they agree at the
		three consecutive values $n = 0, 1, 2$.  Equality for all integers
		$n$ follows.
	\end{proof}
