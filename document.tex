...
\begin{document}
	...
	\subsection{Minimum polynom P(t)}
	\begin{definition}
	$$P(t) = t^3 - e_1 t^2 + e_2 t - e_3$$
	
	Then $x$, $y$ and $z$ are the solutions of equation $P(t) = 0$
	\end{definition}
	...
	\end{theorem}
	\begin{proof}
		From the lemma for the minimum polynom we get that each of $x, y, z$ satisfies $t^3 = e_1 t^2 - e_2 t + e_3$. After multiplication by $t^n$ and using the lemma for isolating single power, we then obtain the result by linearity.
	\end{proof}
	...
	\begin{proof}
		...
		for $f_2$ is just a simple algebra.
	\end{proof}
